...
function material(i){
  const v=values(i+2);
  return `<section class="material" aria-label="${names[i]}进场信息"><div class="material-body"><div class="material-title"><span class="material-icon">${icon(i===0?'bag':'stone')}</span><div class="material-name">${names[i]}</div><span class="arrival-count">进场 <b>${v.selfBatches+v.externalBatches}</b> 批</span></div><div class="material-values"><div><span>累计进场</span><b>${fmt(v.incoming)}</b></div><div><span>理论消耗</span><b>${fmt(v.theory)}</b></div><div class="difference ${v.delta<0?'negative':''}"><span>差值</span><b>${signed(v.delta)}</b></div></div></div></section>`;
}
function render(){
  if(!data)return;
  ...
  $('#dashboard').innerHTML=`<section class="hero" aria-label="${station==='all'?'全部站点':Number(station)+1+'号站'}总览"><div class="hero-top"><span>累计进场总量</span><span class="unit-badge">${station==='all'?'全部站点':Number(station)+1+'# 拌合站'}</span></div><div class="hero-number">${fmt(v.incoming)}<small>t</small></div><div class="arrival-batches"><span>进场批次</span><span><b>${v.selfBatches+v.externalBatches}</b> 批</span></div><div class="hero-bottom"><div><span>理论总消耗 / t</span><strong>${fmt(v.theory)}</strong></div><div><span>进场 − 理论 / t</span><strong class="delta">${signed(v.delta)}</strong></div></div></section><div class="section-head"><h2>材料明细</h2><span>6 类材料 / 单位 t</span></div><p class="section-caption">差值 = 进场 − 理论消耗，不代表库存</p><div class="materials">${names.map((_,i)=>material(i)).join('')}</div><div class="aggregate"><div>集料进场合计<span>石屑 + 各规格碎石，不含水泥</span></div><strong>${fmt(agg.incoming)} <small>t</small></strong></div><details class="notes"><summary>统计口径与数据来源 ⌄</summary><p>进场量和进场批次均为全部有效进场记录合计。差值 = 进场总量 − 理论消耗。差值为统计量差额，不代表实盘库存或损耗率。</p><p>材料用途按施工台账既有筛选规则统计，仅计入有有效代表数量的进场记录。每条有效记录计一个批次，全部站点为 1# 与 2# 之和。</p><p>页面不再定时刷新，打开网页或手动刷新时读取统计表；实时通道不可达时显示同域备用快照，备用同步计划为每 5 分钟一次，可能延迟。数据读取时间与引用更新时间分开显示。打开或刷新网页、下拉时先请求更新引用，确认完成后再读取自用施工台账；引用更新失败时保留上次成功时间，并尝试读取台账当前值。</p><a href="https://www.kdocs.cn/l/clnD16go2o5m" target="_blank" rel="noopener noreferrer">打开施工台账 ↗</a></details>`;
...
document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!pullBusy)updateReferences();});window.addEventListener('online',()=>{if(!pullBusy)updateReferences();});

const updateReferences=createReferenceUpdater((message,error)=>{const el=$("#update-status");el.hidden=false;el.textContent=message;el.classList.toggle("error",!!error);},refresh,undefined,at=>{
  if(Number.isFinite(at)&&at>0){$('#reference-time').textContent='引用更新 '+timeText(new Date(at).toISOString());}
});
updateReferences.readStatus();
updateReferences();
