...
setInterval(()=>{if(!document.hidden&&!pullBusy)updateReferences();},60000);document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!pullBusy)updateReferences();});window.addEventListener('online',()=>{if(!pullBusy)updateReferences();});
